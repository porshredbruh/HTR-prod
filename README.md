...
Проект в архиве содержит:
```
Server-HTR
├── main.py            # Основной скрипт, содержащий логику работы модели/сервера
...
1. Установите Python 3.8+ (или более поздняя версия).  
...
Проект содержит файл requirements.txt со списком необходимых библиотек. Чтобы установить все зависимости, выполните:
```
...
Наиболее простой способ запустить FastAPI-приложение:
...
где:
- main:app — это путь к объекту приложения FastAPI (в данном случае app из файла main.py).
- --host 0.0.0.0 — позволяет слушать запросы на всех сетевых интерфейсах.  
- --port 8000 — стандартный порт для uvicorn. Можно изменить на любой другой.

В консоли вы увидите логи сервера, и если всё в порядке, будет что-то вроде:
...
## Android приложение
1. Скачайте и справьте "YOUR URL" на свой URL в файле `MainActivity.java`

___

Поздравляю! Теперь все готово к использованию!
